...
| `QA_ENABLED` | `true/false`. Включает Q&A-модуль интенсива. |
| `QA_BUTTONS_SHOWN` | `true/false`. Показывать ли главное меню тем кнопками. |
| `QA_FALLBACK_TO_MENU` | `true/false`. При непонимании возвращает пользователя в меню тем. |
...
## Производственный интенсив

### Воронка «📝 Записаться»

- Вход — кнопка «📝 Записаться» в меню интенсива после подтверждения подписки (`sub_ok`).
- Запрос телефона сопровождается кнопками «📱 Отправить телефон», «Ввести номер вручную», «Отмена» и подсказкой формата `+7XXXXXXXXXX`.
- Телефон нормализуется в формат `+7XXXXXXXXXX` (поддерживаются номера, начинающиеся с `8`, `7` или `9`).
- Пользователь получает подтверждение «Спасибо! Свяжемся, уточним точные даты и оплату 👌».
- Алёрт координатору содержит заголовок «🆕 Новый лид (Интенсив)», ссылку на пользователя, кампанию, телефон (с маской, если `ALERTS_MASK_PHONE=true`), локальное время (`ALERTS_TZ` + `ALERTS_TIME_FORMAT`) и строку UTC. При ошибке записи в Google Sheets добавляется строка `Ошибка: DB_WRITE_FAILED`.
- В Google Sheets (`leads`) сохраняются поля `user_id`, `@username` (нижний регистр), нормализованный `phone`, `campaign`, `created_at` (UTC ISO), опциональный `created_at_msk`, `status="new"`. При `LEADS_UPSERT=true` строка обновляется по `(user_id, campaign)` с заполнением `updated_at`.
- В лист `events` пишутся шаги `lead_init`, `phone_received`, `sheets_write_ok`/`sheets_write_failed`, `lead`, `alert_sent`/`alert_skipped`, `lead_duplicate`. В `meta_json` нет полного телефона.
- Повторные заявки подавляются в течение 60 секунд — бот отвечает «Заявка уже принята 👍».

### Q&A-модуль

- Меню тем (при `QA_BUTTONS_SHOWN=true`): 📌 Что за интенсив, 📅 Расписание, 🏭 Программа, 📍 Где проходит, 👩‍🔬 Спикеры и темы, 🍽 Что включено, 💳 Стоимость, 🎓 Сертификат, 📝 Записаться.
- Каждое сообщение содержит короткий ответ и CTA «📝 Записаться» (кнопка + текст), а также кнопку «◀️ Назад» для возврата к меню.
- Свободные вопросы распознаются по ключевым словам («расписание», «где», «стоимость», «спикеры», «программа», «еда/питание», «сертификат», «записаться» и др.).
- Анти-спам: бот отвечает не чаще чем раз в `QA_RATE_LIMIT_SECONDS` секунд на пользователя.
- При неизвестном вопросе: если `QA_FALLBACK_TO_MENU=true`, отправляется `QA_UNKNOWN_ANSWER` и показывается меню тем, иначе предлагается сразу записаться.
- Нажатия «📝 Записаться» фиксируются как `qa_clicked_cta` с указанием темы, после чего запускается воронка.

### Алёрты и время

- Алёрты используют локаль `ALERTS_TZ` + `ALERTS_TIME_FORMAT` и всегда включают строку с UTC-временем.
